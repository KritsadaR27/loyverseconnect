...
import { ChevronDownIcon, ChevronUpIcon, LightningBoltIcon,DotsVerticalIcon } from '@heroicons/react/solid';
...
const CreatePO = () => {
        ...
        const loadItems = async () => {
            ...
            if (fetchedItems) {
                ...
                fetchedItems.forEach(item => {
                    ...
                    if (storeName === "ลุงรวย รถส่งของ" || storeName === "สาขาอื่นๆ") return;
                    ...
                    stockTotals[itemName].recommendation = calculateRecommendation(stockTotals[itemName]);
                    stockTotals[itemName].reserve = reserveValues[item.item_id] || 0; // ตั้งค่าจาก reserveValues
                }); 
    ...
    const calculateRecommendation = (item) => {
        const reserve = reserveValues[item.item_id] || 0;
        const remainingStock = item.total_stock - reserve - (item.sales.sat || 0) - (item.sales.sun || 0);
