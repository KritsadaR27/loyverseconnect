...
func SyncMasterDataHandler(w http.ResponseWriter, r *http.Request) {
	...
	// เคลียร์ข้อมูลเก่า
	if err := database.ClearOldMasterData(dbConn); err != nil {
...
func SyncReceipts(dbConn *sql.DB) error {
	// เคลียร์ข้อมูลเก่า
	if err := database.ClearOldReceiptsData(dbConn); err != nil {
		return err
	}
	...
	for {
		...
		receipts, nextCursor, err := loyservices.FetchReceiptsBatch(cursor, limit)
		if err != nil {
		...
		if err := database.SaveReceipts(dbConn, receipts); err != nil {
...
func SyncInventoryLevels(db *sql.DB) error {
	// เคลียร์ข้อมูลเก่า
	if err := database.ClearOldInventoryLevelsData(db); err != nil {
		return err
	}

	// ดึงข้อมูล inventory levels จาก Loyverse API
	inventoryLevels, err := loyservices.FetchInventoryLevels()
	if err != nil {
		return err
	}

	// บันทึกข้อมูล inventory levels ลงฐานข้อมูล
	if err := database.SaveInventoryLevels(db, inventoryLevels); err != nil {
		return err
	}
